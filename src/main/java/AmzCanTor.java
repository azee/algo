 ...
 */
public class AmzCanTor {

    static class LogEntry {
        public LogEntry(String date, String id) {
            this.date = date;
            this.id = id;
        }
        ...
        String getCustomerId(){return id;};
    }


    public static void main(String... args) throws IOException {
          int result = countRepeatVisitors(Arrays.asList(
                  new LogEntry("Bob", "1"),
                  new LogEntry("Bob", "2"),
                  new LogEntry("Bob1", "1")
          ));
    ...
    static int countRepeatVisitors(List<LogEntry> logEntries) {
        ...
        logEntries.forEach(logEntry -> {
            Set<String> dates = usersMap.get(logEntry.getCustomerId());
            if (dates == null){
                dates = new HashSet<String>();
                usersMap.put(logEntry.getCustomerId(), dates);
            }
            dates.add(logEntry.getDate());
        });
        return  Math.toIntExact(usersMap.entrySet().stream().filter(entry -> entry.getValue().size() > 1).count());
    }
